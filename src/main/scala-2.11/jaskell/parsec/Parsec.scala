...
trait Parsec[E, +T] {
  ...
  def `<?>`[U >: T](message: String): Parsec[E, U] = Parsec((s: State[E]) => {
  ...
  def map[B](f: T => B): Parsec[E, B] = Parsec((s: State[E]) => self(s).map(f))

  def <:>[B](f: T => B): Parsec[E, B] = self.map(f)

  def flatMap[B](f: T => Parsec[E, B]): Parsec[E, B] = Parsec((s: State[E]) =>
    for {
      a <- self(s)
      func = f(a)
      x <- func(s)
  ...
  def liftA2[B, C](f: (T, B) => C): Parsec[E, B] => Parsec[E, C] = (p: Parsec[E, B]) => Parsec((s: State[E]) =>
    for {
      a <- self(s)
      b <- p(s)
    } yield f(a, b))
