 ...
 */
case class Chars1In(elements: String, caseSensitive: Boolean=true) extends Parsec[Char, String] {
  import jaskell.Monad.Implicits._
  import jaskell.parsec.Parsec.Implicits._

  val parser: Parsec[Char, Seq[Char]] = new Many1(new ChIn(elements, caseSensitive))
