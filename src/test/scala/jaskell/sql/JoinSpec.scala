...
class JoinSpec extends AnyFlatSpec with Matchers {
  ...
  Using(DriverManager.getConnection(url)) { conn =>
    conn.prepareStatement("create table test(id integer primary key autoincrement, " + "pid integer references test(id), " + "content text)").execute
    ...
    "LeftJoin1" should "Test left join" in {
      val q: Query = select("r.id")
        ...
        .where(n("r.id").isNull)
      val re = q.scalar(conn)
      re.isDefined should be(true)
    }
  }
}
